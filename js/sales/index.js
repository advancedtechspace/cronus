...
function searchSales(value, data) {
  ...
  for (sale of d) {
    ...
    trows += `
      <tr>
        <td><a href='${api_url}/cronus/sales/factura/${_id}?u=${localStorage.getItem('user')}' style='font-size: 14px;' target='_blank'>${_id}</a></td>
        <td>
        ...
        <td>${cart.length}</td>
        <td width='20%'>
          <a href='${api_url}/cronus/sales/factura/${_id}?u=${localStorage.getItem('user')}'><button class="btn-circle btn-circle-show" id="staff-edit-0" target='_blank'><i class='la la-eye'></i></button></a>
  ...
  for (let i = limInf; i < limSup; i++) {
    ...
    trows += `
          <tr>
            ...
            <td width='20%'>
            <a href='${api_url}/cronus/sales/factura/${_id}?u=${localStorage.getItem('user')}' target='_blank'><button class="btn-circle btn-circle-show" id="staff-edit-0"><i class='la la-eye'></i></button></a>
              <button class="btn-circle btn-circle-delete btn-delete-staff" id="${_id}"><i class='la la-trash'></i></button>
            </td>
          </tr>
        `;
